...
{
    public partial class LoginClerks : Form
    {
        public LoginClerks()
        {
            InitializeComponent();
        ...
        private void LoginButton_Click(object sender, EventArgs e)
        {
            ...
            DataBaseClerks db = new DataBaseClerks();

            DataTable table = new DataTable();

            MySqlDataAdapter adapter = new MySqlDataAdapter();

            MySqlCommand command = new MySqlCommand("SELECT * FROM `clerk` WHERE `login` = @uL AND `password` = @uP", db.getConnection());
